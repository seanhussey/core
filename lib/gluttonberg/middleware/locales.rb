module Gluttonberg
  module Middleware
    class Locales
      ...
      def call(env)
        path = env['PATH_INFO']
        unless path =~ /^\/admin/ || path.start_with?("/stylesheets")  || path.start_with?("/javascripts")   || path.start_with?("/images") ||  path.start_with?("/gluttonberg")  || path.start_with?("/assets")  || path.start_with?("/user_asset")
